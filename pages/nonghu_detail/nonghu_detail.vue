	...
	export default {
		...
		methods:{
			...
			selPoints(item){
				if(item.text == "农技示范记录"){
					uni.navigateTo({
						url:`../nong_technology_record/nong_technology_record?user_id=${this.user_id}&from=${"nonghu_detail"}`
					})
				}else{
					uni.navigateTo({
						url:`../technology_share/technology_share?user_id=${this.user_id}&from=${"nonghu_detail"}`
					})
				}
			},
			getPerInfo(user_id){
